...
altair_bar = alt.Chart(plot_data).mark_bar().encode(x=alt.X('week(bar_labels):T',sort=None,axis=alt.Axis(title='')),y=alt.Y('avg_prices:Q',axis=alt.Axis(title='',format='.2f')),color=alt.Color('flip_arange_values',legend=None,scale=alt.Scale(scheme='redyellowgreen')))
altair_line = alt.Chart(plot_data).mark_line().encode(x=alt.X('week(bar_labels):T',sort=None),y='avg_prices:Q',color=alt.value('orange'))
altair_line_linreg = alt.Chart(plot_data).mark_line().encode(x=alt.X('week(bar_labels):T',sort=None),y='lin_reg_values:Q',color=alt.value('red'))
