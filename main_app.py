...
altair_bar = alt.Chart(plot_data).mark_bar(band=1).encode(x=alt.X('bar_labels',sort=None,axis=alt.Axis(title='')),y=alt.Y('avg_prices:Q',axis=alt.Axis(title='',format='.2f')),color=alt.Color('flip_arange_values',legend=None,scale=alt.Scale(scheme='redyellowgreen')))
