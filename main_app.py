...
with tab5:
        ...
        if display_experimental == 'EXPERIMENTAL':
                ...
                for noun in noun_list:
                        veg_count = veg_df.loc[noun in veg_df].shape[0]
                        nonveg_prop = nonveg_df.loc[noun in nonveg_df].shape[0]/nonveg_df.shape[0]
                        veg_prop = veg_count/veg_df.shape[0]
                        prop_dict[noun] = (nonveg_prop, veg_prop)
                        if (nonveg_prop >= 1.5*veg_prop) and (veg_count >= 5):
                                st.write(noun)
